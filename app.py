...
if not st.session_state.logged_in:
    ...
    # خواندن کلیدها از Environment Variables جهت امنیت در استقرار ابری
    OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY", "sk-jOn337n0y1yYP7kWTQVFRzuCsvfXlA5Y56kUDkRaQeoqhORC")
    BASE_URL = os.environ.get("BASE_URL", "https://api.gapgpt.app/v1")
        ...
        if text_prompt:
            ...
            with st.spinner("🗣️ Transcribing voice..."):
                try:
                    ...
                    st.error(f"⚠️ خطا در تبدیل صوت به متن: {e}")
                    final_prompt = None
...
Helpful Answer in English:"""
                    ...
                    answer = ""
                    try:
                        # تلاش برای ارتباط با سرور و گرفتن جواب
                        result = llm.invoke(chat_history + [human_msg])
                        answer = result.content
                    except Exception as e:
                        # در صورت بروز خطا در سرور پروکسی، برنامه هنگ نکرده و این پیام را نمایش می‌دهد
                        answer = f"⚠️ **خطا در ارتباط با سرور هوش مصنوعی:**\n\n`{str(e)}`\n\nلطفا لحظاتی بعد مجددا تلاش کنید."
                    
                    st.markdown(answer)
                    
                    sources_for_db = []
                    if len(source_docs) > 0 and not answer.startswith("⚠️"):
                    ...
                    # ذخیره در دیتابیس و مموری
                    auth.save_message(st.session_state.current_chat_id, "assistant", answer, sources_for_db)
                    
                    # در صورتی که خطا رخ نداده بود، دیالوگ را در حافظه Context ذخیره کن
